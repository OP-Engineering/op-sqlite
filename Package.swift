  ...
  targets: binaryTargets + [
    .target(
      ...
      cSettings: defineCSettings + [
        ...
        // slices, so any one slice works). turso_sync.h then internally
        // does a BARE `#include <turso.h>` (no turso_sdk_kit/ prefix) —
        // framework-header lookup doesn't cover that form, so the same
        // Headers/ dir also needs a plain -I search path.
        + (useTurso
          ? [.headerSearchPath("ios/turso_sdk_kit.xcframework/ios-arm64/turso_sdk_kit.framework/Headers")]
          : [])
        + [
          .unsafeFlags(

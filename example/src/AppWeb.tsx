...
export default function AppWeb() {
  ...
  const withDb = useCallback(
    async <T,>(work: (db: Awaited<ReturnType<typeof openAsync>>) => Promise<T>) => {
  ...
  const refreshTableInfo = useCallback(async () => {
    return withDb(async (db) => {
      const [schemaResult, rowsResult] = await Promise.all([
        ...
        db.execute('SELECT id, label, created_at FROM web_notes ORDER BY id DESC LIMIT 20'),
      ]);
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.panel}>
        ...
        <View style={styles.row}>
          <Pressable style={styles.button} disabled={loading} onPress={insertNote}>
      ...
      <View style={styles.panel}>
        ...
        <View style={styles.row}>
          <Pressable style={styles.button} disabled={loading} onPress={refreshTableInfo}>
            <Text style={styles.buttonText}>Refresh table</Text>
          </Pressable>
          <Pressable style={styles.buttonDanger} disabled={loading} onPress={clearRows}>
        ...
        {columns.map((column) => (
          <View key={column.cid} style={styles.tableRow}>
            ...
            <Text style={[styles.rowCell, styles.flagCell]}>{column.notnull}</Text>
        ...
        {notes.length === 0 ? (
          ...
          notes.map((entry) => (
            <View key={entry.id} style={styles.tableRow}>
              ...
              <Text style={[styles.rowCell, styles.nameCell]}>{entry.label}</Text>
              <Text style={[styles.rowCell, styles.dateCell]}>{entry.created_at}</Text>
            </View>
          ))
        )}
      </View>

...
describe("Queries tests", () => {
  ...
  it("Can create multiple connections to same db", async () => {
    ...
    const promises = [db.execute("SELECT 1"), db2.execute("SELECT 1"), db3.execute("SELECT 1")];
  ...
  it("Trying to pass object as param should throw", async () => {
    try {
      ...
      expect(e.message.includes("Object is not an ArrayBuffer or ArrayBuffer view")).toEqual(true);
    }
  });
  ...
  it("executeSync", () => {
    ...
    const res2 = db.executeSync('INSERT INTO "User" (id, name, age, networth) VALUES(?, ?, ?, ?)', [
      id,
      name,
      age,
      networth,
  ...
  it("Query without params", async () => {
    ...
    const age = chance.integer();
    const networth = chance.floating();
    await db.execute("INSERT INTO User (id, name, age, networth) VALUES(?, ?, ?, ?)", [
      id,
      name,
      age,
      networth,
  ...
  it("Preserves non-ASCII strings when binding params (issue #417)", async () => {
    ...
    await db.execute("DROP TABLE IF EXISTS UnicodeRepro;");
    await db.execute("CREATE TABLE UnicodeRepro (value TEXT) STRICT;");
    await db.execute("INSERT INTO UnicodeRepro (value) VALUES (?)", [value]);

    const result = await db.execute("SELECT value FROM UnicodeRepro;");
  ...
  it("Create fts5 virtual table", async () => {
    ...
    await db.execute("CREATE VIRTUAL TABLE fts5_table USING fts5(name, content);");

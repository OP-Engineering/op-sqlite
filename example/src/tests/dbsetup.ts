...
it("Can attach/dettach database", () => {
  ...
  db.executeSync("CREATE TABLE IF NOT EXISTS attach2.test (id INTEGER PRIMARY KEY);");
...
it("Neutralizes SQL injection payload in attach path", () => {
  ...
  db.executeSync("CREATE TABLE IF NOT EXISTS quirky.canary (id INTEGER PRIMARY KEY);");
